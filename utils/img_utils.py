...
def draw_boxes_old(image, labels, boxes):
    ...
    for i, box in enumerate(boxes):
        ...
        for i in range(thickness):
            draw.rectangle(
                [left + i, top + i, right - i, bottom - i], outline=(255, 0, 255)
            )
        draw.rectangle(
...
def draw_boxes(image, det_out, from_path=True, out_file="temp_bbox_img.png"):
    ...
    font_path = os.path.join(
        os.path.dirname(__file__), "../models/yolov3/model_data/FiraMono-Medium.otf"
    )
    font = ImageFont.truetype(
        font=font_path, size=np.floor(3e-2 * source_img.size[1] + 0.5).astype("int32")
    )
...
def visualize_features(
    ...
    for feature_idx, temp_feature in enumerate(tqdm(intermediate_features)):
        ...
        temp_file_path = os.path.join(
            output_dir, file_prefix + "_{0:03d}.png".format(feature_idx)
        )
...
def visualize_features_compare(
    ...
    for feature_idx, (temp_ori_feature, temp_adv_feature) in enumerate(
        tqdm(zip(ori_features, adv_features))
    ):
        if only_first_channel and feature_idx != 0:
            break
        temp_file_path = os.path.join(
